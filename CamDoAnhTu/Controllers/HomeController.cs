    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult LoadCustomer(int? pageSize, int page = 1)
        {
            ...
            using (CamdoAnhTuEntities1 ctx = new CamdoAnhTuEntities1())
            {
                ...
                list1 = ctx.Customers.Where(p => p.CodeSort == null).ToList();
        ...
        public ActionResult AddCustomer(Customer model, HttpPostedFileBase fuMain)
        {
            using (CamdoAnhTuEntities1 ctx = new CamdoAnhTuEntities1())
            {
                ...
                if ((model.Code[0] >= 'A' && model.Code[0] <= 'Z') || (model.Code[0] >= 'a' && model.Code[0] <= 'z'))
                {
                    int code = Int32.Parse(model.Code.Substring(1, model.Code.Length - 1));
                    if (model.Code[0] == 'A')
                    {
                        model.CodeSort = (model.CodeSort == null ? code + 1000 : model.CodeSort);
                    }
                    else if (model.Code[0] == 'B')
                    {
                        model.CodeSort = (model.CodeSort == null ? code + 2000 : model.CodeSort);
                    }
                    else if (model.Code[0] == 'C')
                    {
                        model.CodeSort = (model.CodeSort == null ? code + 3000 : model.CodeSort);
                    }
                    else if (model.Code[0] == 'D')
                    {
                        model.CodeSort = (model.CodeSort == null ? code + 4000 : model.CodeSort);
                    }
                    else if (model.Code[0] == 'E')
                    {
                        model.CodeSort = (model.CodeSort == null ? (code + 5000) : model.CodeSort);
                    }
                }
                else
                {
        ...
        //private void WriteHistory(Customer p, int type)
        //{
        //    StringBuilder str = new StringBuilder();
